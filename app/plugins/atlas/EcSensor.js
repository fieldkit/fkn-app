...
class ConductivityProbeTypeStep extends ScriptStep {
    onSetProbe(k) {
        ...
        atlasSetProbeType(
            SensorType.values.EC,
            atlasState.commands.Ec.SetProbe[k],
            k
        );
    }
    ...
    renderStep() {
        ...
        return (
            <View>
                ...
                <View style={atlasStyles.buttons.largeButton}>
                    <Button
                        title="K = 0.1"
                        onPress={() => this.onSetProbe("0.1")}
                    />
                </View>
                <View style={atlasStyles.buttons.largeButton}>
                    <Button
                        title="K = 1"
                        onPress={() => this.onSetProbe("1")}
                    />
                </View>
                <View style={atlasStyles.buttons.largeButton}>
                    <Button
                        title="K = 10"
                        onPress={() => this.onSetProbe("10")}
                    />
                </View>
...
class TemperatureCompensationStep extends ScriptStep {
    ...
    renderStep() {
        ...
        return (
            <View>
                <Paragraph>
                    Temperature has a significant effect on conductivity
                    readings. The EZO™ Conductivity circuit has its temperature
                    compensation set to 25˚ C as the default. If the calibration
                    solution is not within 5˚ of 25˚ C, check the temperature
                    chart on the side of the calibration bottle, and calibrate
                    to that value.
                </Paragraph>
        ...
        return (
            <AtlasScript onCancel={() => onCancel()}>
                ...
                <InstructionsStep>
                    <ReadingsDisplay atlasState={atlasState} />
                    <Paragraph>
                        Please ensure that the probe is dry before we perform
                        the dry calibration.
                    </Paragraph>
                </InstructionsStep>
                ...
                <AtlasCalibrationCommandStep
                    ...
                    command={atlasState.commands.Ec.CalibrateDry}
                    atlasState={atlasState}
                ...
                <InstructionsStep>
                    ...
                    <Paragraph>
                        ...
                        cup. Shake the probe to make sure you do not have
                        trapped air bubbles in the sensing area. You should see
                        readings that are off by 1 – 40% from the stated value
                        of the calibration solution. Wait for readings to
                        stabilize (small movement from one reading to the next
                        is normal).
                    </Paragraph>
                ...
                <AtlasCalibrationCommandStep
                    sensor={SensorType.values.EC}
                    command={atlasState.commands.Ec.CalibrateLow}
                    atlasState={atlasState}
                    atlasCalibrate={atlasCalibrate}
                >
                    <ReadingsDisplay atlasState={atlasState} />
                ...
                <AtlasCalibrationCommandStep
                    sensor={SensorType.values.EC}
                    command={atlasState.commands.Ec.CalibrateHigh}
                    atlasState={atlasState}
                    atlasCalibrate={atlasCalibrate}
                >
                    <ReadingsDisplay atlasState={atlasState} />
