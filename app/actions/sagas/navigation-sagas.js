...
function connectedRoute(route) {
    if (
        route.routeName == "Welcome" ||
        route.routeName == "About" ||
        route.routeName == "EasyModeWelcome" ||
        route.routeName == "Connecting" ||
        route.routeName == "Browser"
    ) {
        return false;
    }
    return true;
...
export function* navigateHomeOnConnectionLost() {
    yield takeLatest(Types.FIND_DEVICE_LOST, function*(lostDevice) {
        ...
        if (
            ...
            deviceStatus.connected.key === lostDevice.address.key
        ) {
